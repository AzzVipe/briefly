				...
				<div v-if="!hasMessages && !isLoadingMessages" class="welcome-area">
					<div class="welcome-content">
						...
						<h1 class="welcome-title">
							{{
								activeConversationId ? "Ready to chat" : "Start a conversation"
							}}
						</h1>

						<p class="welcome-desc">
							<template v-if="!activeConversationId">
								Send a message to create a new conversation. Upload PDFs to add
								context.
							</template>

							<template v-else-if="!attachedDocs.length">
								No documents attached yet. Upload or attach PDFs above to scope
								retrieval to this conversation.
							</template>
...
<script setup lang="ts">
	...
	const { conversations, activeConversationId, setActive, upsertConversation } =
		useConversations();
